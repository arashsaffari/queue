...
class Delete extends BaseCommand
{
	...
	protected $name         = 'queue:delete';
	protected $description  = 'Delete a queue task';
	protected $usage        = 'queue:delete';
	protected $arguments    = [
		'id'	=> 'queue task ID'
	];
	protected $options 		= [
	...
	public function run(array $params)
	{
		...
		if (!$this->quiet) CLI::write('delete the queue task ...', 'white');
		...
		if($this->task_id){
			...
			//
			$this->deletedStatus = $this->queue->getPersistor()->deleteTaskWithID($this->task_id,null,null);
			...
			if ($this->deletedStatus){
				...
				if (!$this->quiet) CLI::error('Either the task is not in the database.');
